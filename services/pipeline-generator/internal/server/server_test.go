...
func TestGenerateNextJSSucceeds(t *testing.T) {
	...
	payload := map[string]any{
		...
		"stack": map[string]any{"runtime": "node", "framework": "nextjs"},
	}
	...
	if response.Template != "github-actions-nextjs" {
		t.Fatalf("expected template %q, got %q", "github-actions-nextjs", response.Template)
	}
}
...
func TestGenerateUnsupportedStackReturns422(t *testing.T) {
	...
	payload := map[string]any{
		...
		"provider":     "github-actions",
		"stack": map[string]any{"runtime": "go", "framework": "gin"},
	}
	body, err := json.Marshal(payload)
	if err != nil {
		t.Fatalf("marshal payload: %v", err)
